...
void LoadImages(const string &strSequence, vector<string> &vstrImageFilenames,
                vector<double> &vTimestamps, const int &num_frames);
...
int main(int argc, char **argv)
{
    ...
    LoadImages(string(argv[3]), vstrImageFilenames, vTimestamps, stoi(argv[4]));
...
void LoadImages(const string &strPathToSequence, vector<string> &vstrImageFilenames, vector<double> &vTimestamps, const int &num_frames)
{
    ...
    for (int i = 1; i <= num_frames; i++) {
        vTimestamps[i-1] = ((double)i-1) / 1000.0; //1000 fps
